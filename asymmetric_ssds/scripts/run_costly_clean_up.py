...
def train(config, checkpoint=None, num_iterations=50000):
    ray.init(address=None, log_to_driver=False, num_cpus=8)

    algo = config.build(logger_creator=custom_log_creator(result_directory, "pot"))
...
def start_training(environment_config):
    ...
    policies = {}
    for i in range(len(environment_config["roles"])):
    ...
    config = (
        DQNConfig()
        .environment(env="costly_clean_up", env_config=environment_config)
        .training(
            gamma=0.99,
            lr=1e-05,
        ...
        .rollouts(num_rollout_workers=2, num_envs_per_worker=1, batch_mode="complete_episodes", rollout_fragment_length=100)
        .framework("torch")
        .evaluation(
...
if __name__ == "__main__":
    ...
    register_env("costly_clean_up", lambda config: custom_env_creator(config))

    substrate_name = "costly_clean_up"
